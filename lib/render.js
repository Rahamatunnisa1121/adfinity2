...
function renderHeader(options = {}) {
  ...
  const bottomRight = contactPage || aboutPage
    ? ''
    : detailStats
      ? `<li class=nav-item><a class="nav-link nav-view" href=# onclick=return!1><img src=/resources/images/Enlarge-Eye-icon.png alt="">${detailStats.views}</a></li><li class=nav-item><a id=nav-like class="nav-link nav-like" href=#><img src=/resources/images/1x1.png alt="">${detailStats.likes}</a></li>`
      : itemCount != null
        ? `<li class=nav-item><a class="nav-link nav-count" href=# onclick=return!1>Showing ${itemCount} items</a></li>`
        : '';
  ...
  const categoryMenu = `<div class="dropdown-menu adf-category-dropdown adf-choose-category-menu" aria-labelledby=adfChooseCategory>${categoryLinks}<div class=dropdown-uparrow><img src=/resources/images/catagory-uparrow.png alt=""></div></div>`;

  const chooseCategoryDesktop = `<li class="nav-item dropdown adf-cat-choose-item"><a class="nav-link nav-category dropdown-toggle" href=# id=adfChooseCategory data-toggle=dropdown aria-haspopup=true aria-expanded=false>Choose Category</a>${categoryMenu}</li>`;

  const chooseCategoryMobile = `<li class="nav-item dropdown adf-nav-category-item adf-nav-category-mobile"><a class="nav-link adf-nav-btn adf-nav-btn-category dropdown-toggle" href=# id=adfCategoryToggleMobile data-toggle=dropdown aria-haspopup=true aria-expanded=false><span>Choose Category</span></a><div class="dropdown-menu adf-nav-dropdown adf-category-dropdown" aria-labelledby=adfCategoryToggleMobile>${categoryLinks}</div></li>`;

  return `<header><div id=header><div id=hd-top><nav class="navbar navbar-expand-lg navbar-light adf-navbar">${brandLogo}<div class=adf-nav-wrap><ul class="navbar-nav adf-nav-primary ml-auto"><li class="nav-item"><a class="nav-link nav-about${aboutActive}" href=/about><span>About</span></a></li><li class="nav-item"><a class="nav-link nav-contact${contactActive}" href=/contact><span>Contact</span></a></li>${chooseCategoryMobile}</ul></div></nav></div><div id=hd-bot><nav class="navbar navbar-expand-lg navbar-light adf-navbar-bot"><ul class="navbar-nav adf-cat-choose">${chooseCategoryDesktop}</ul><ul class="navbar-nav nav-right ml-auto">${bottomRight}</ul></nav></div></div></header>`;
}
...
function renderListingPage(options) {
  ...
  return `${renderPageHead(SITE.title, SITE.description, null, activeCategory ? `/${activeCategory}` : '/', '', pageKeywords)}<body><div id=wrapper>${renderHeader({ categories, activeCategory, activeHome, itemCount: total })} <div id=main-wrp class=mt140><div id=masonry-wrp data-masonry='{"itemSelector": ".items", "transitionDuration": 0, "columnWidth": ".items"}'><div class=items></div></div><input type=hidden id=page value="${page}"> <input type=hidden id=total value="${total}"></div>  ${renderFooter()} ${renderFloatingSocial()} </div></body>${renderTail()}`;
}
...
function renderDetailPage(item, options) {
  ...
  const detailJs =
    '<script type=text/javascript src="/resources/js/detail-engagement.js"></script>';

  return `${renderPageHead(pageTitle, description, shareImage, itemPath)}<body><div id=wrapper>${renderHeader({ categories, activeCategory: item.category, detailStats: stats })} <div id="main-wrp"><div class="enlarge-wrp"><div class="adf-detail-frame">${prevArrow}<div class="enlarge-icons text-center">      ${renderGallery(item)}   </div>${nextArrow}</div></div></div> ${renderFloatingSocial()} </div></body>${renderTail(detailJs)}`;
}
...
function renderContactPage(options) {
  ...
  const extraCss = '<link href="/resources/css/contact.css?v=14" rel=stylesheet type=text/css>';
  const extraJs =
  ...
  return `${renderPageHead(pageTitle, description, null, '/contact', extraCss, SITE.seo.contact.keywords)}<body><div id=wrapper>${renderHeader({ categories, activeContact: true, contactPage: true })} ${body} ${renderFooter()} ${renderFloatingSocial()} </div></body>${renderTail(extraJs)}`;
}
...
function renderAboutPage(options) {
  ...
  const extraCss = '<link href="/resources/css/about.css?v=16" rel=stylesheet type=text/css>';
  ...
  return `${renderPageHead(pageTitle, description, null, '/about', extraCss, `${SITE.seo.about.keywords}, ${SITE.seo.services.keywords}`)}<body><div id=wrapper>${renderHeader({ categories, activeAbout: true, aboutPage: true })} ${body} ${renderFooter()} ${renderFloatingSocial()} </div></body>${renderTail(extraJs)}`;
}
